<template>
    <div>
        ...
        <div v-else class="grid xl:grid-cols-4 lg:grid-cols-3 md:grid-cols-2 grid-cols-1 mt-10 gap-[30px]">
            <div v-for="license in setlicenseData.slice(0, 8)" :key="license.id" class="group relative overflow-hidden p-2 rounded-lg bg-white dark:bg-slate-900 border border-gray-100 dark:border-gray-800 hover:shadow-md dark:shadow-md hover:dark:shadow-gray-700 transition-all duration-500 hover:-mt-2 h-fit flex flex-col justify-between min-h-[410px]">
                <div class="flex-1 flex flex-col justify-between">
                    <div>
                        <div class="relative overflow-hidden rounded-lg">
                            <img :src="license.image_url_1" class="rounded-lg shadow-md dark:shadow-gray-700 group-hover:scale-110 transition-all duration-500" alt="">
                        </div>
                        <div class="absolute -bottom-20 group-hover:bottom-1/2 group-hover:translate-y-1/2 start-0 end-0 mx-auto text-center transition-all duration-500">
                            <router-link :to="{name: 'details-license', params: {id: license.id}}" class="btn btn-sm rounded-full bg-violet-600 bg-opacity-80 hover:bg-violet-700 border-violet-600 hover:border-violet-700 text-white mr-2 min-w-[100px] flex justify-center items-center"><i class="mdi mdi-eye"></i> Lihat Detail</router-link>
                            <router-link :to="{name: 'creator-chat', params: {id: license.id}}" class="btn btn-sm rounded-full bg-green-600 bg-opacity-80 hover:bg-green-700 border-green-600 hover:border-green-700 text-white min-w-[100px] flex justify-center items-center"><i class="mdi mdi-chat"></i> Chat Penjual</router-link>
                        </div>
                        ...
                        <div v-if="license.showdate === true" class="absolute bottom-6 start-0 end-0 mx-auto text-center bg-gradient-to-r from-violet-600 to-red-600 text-white inline-table text-lg px-3 rounded-full justify-center z-10">
                            <i class="uil uil-clock align-middle me-1 text-center"></i>  <small id="auction-item-1" class="font-bold"> {{ license.remaining?.days + " : " + license.remaining?.hours + " : " + license.remaining?.minutes + " : " + license.remaining?.seconds }}</small>
                        </div>
                    </div>
                    <div class="mt-3">
                        <div class="my-3 flex items-center gap-3">
                            <div>
                                ...
                                <div class="flex items-center gap-2 text-xs text-slate-400 mt-1">
                                    ...
                                    <span v-if="license.posting" class="text-xs text-slate-400 italic">{{ license.posting }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="flex justify-between items-center p-2 bg-gray-50 dark:bg-slate-800 rounded-lg shadow dark:shadow-gray-700 mt-1">
                    ...
                    <span class="text-[18px] font-bold text-violet-700 flex items-center"><i class="mdi mdi-cash mr-1"></i>{{ formatRupiah(license.price) }}</span>
                </div>
            </div><!--end content-->
        </div><!--end grid-->
