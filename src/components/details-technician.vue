      ...
      <div class="flex-1">
        ...
        <div class="bg-gradient-to-r from-violet-600 to-blue-400 rounded-2xl shadow-lg p-8 flex flex-col md:flex-row gap-8 items-center relative overflow-hidden">
          ...
          <div class="flex-1 text-center md:text-left text-white">
            ...
            <p class="text-blue-100 text-lg mt-1">{{ technician?.title || 'Teknisi Smartphone Profesional' }}</p>
            ...
            <div class="flex flex-wrap gap-2 mt-4 justify-center md:justify-start">
              ...
              <a :href="technician?.youtube || '#'" class="btn btn-icon btn-sm rounded-full bg-red-600 hover:bg-red-700 text-white" title="Youtube" target="_blank"><i class="mdi mdi-youtube"></i></a>
              <a :href="technician?.instagram || '#'" class="btn btn-icon btn-sm rounded-full bg-gradient-to-tr from-purple-600 to-yellow-400 hover:from-pink-600 hover:to-yellow-600 text-white" title="Instagram" target="_blank"><i class="mdi mdi-instagram"></i></a>
              <a :href="technician?.facebook || '#'" class="btn btn-icon btn-sm rounded-full bg-blue-600 hover:bg-blue-800 text-white" title="Facebook" target="_blank"><i class="mdi mdi-facebook"></i></a>
              <a :href="technician?.whatsapp || '#'" class="btn btn-icon btn-sm rounded-full bg-green-500 hover:bg-green-700 text-white" title="WhatsApp" target="_blank"><i class="mdi mdi-whatsapp"></i></a>
            </div>
          </div>
        </div>
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-6 items-stretch">
          <div v-for="(item, i) in stats" :key="i" class="bg-violet-50 dark:bg-slate-800 rounded-lg p-4 flex flex-col items-center h-full">
            <span :class="item.icon + ' text-4xl text-violet-600 mb-2'"></span>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-xl shadow p-6 mt-6">
          ...
          <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div v-for="(service, idx) in services" :key="idx" class="relative border border-violet-200 dark:border-violet-700 rounded-2xl p-5 bg-white/70 dark:bg-slate-800/80 backdrop-blur-md shadow-lg hover:scale-[1.04] hover:shadow-2xl transition-all duration-300 flex flex-col h-full overflow-hidden">
              <!-- Ribbon label -->
              <div class="absolute top-0 left-0 bg-violet-600 text-white text-xs font-bold px-3 py-1 rounded-br-xl shadow-lg z-10">Service</div>
              <div class="flex items-center gap-3 mb-2 mt-2">
                <span class="inline-flex items-center justify-center w-12 h-12 rounded-xl bg-gradient-to-tr from-violet-200 to-violet-400 text-violet-700 text-3xl shadow-lg"><i :class="service.icon"></i></span>
                <h4 class="font-semibold text-violet-700 text-lg">{{ service.name }}</h4>
              </div>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-xl shadow p-6 mt-6">
          ...
          <h4 class="text-lg font-semibold mb-3 flex items-center gap-2">
            <i class="mdi mdi-hammer-wrench text-violet-600"></i> Hasil Pekerjaan
          </h4>
          ...
          <!-- Galeri Workshop -->
          <h4 class="text-lg font-semibold mb-3 flex items-center gap-2">
            <i class="mdi mdi-store-outline text-violet-600"></i> Galeri Toko / Workshop
          </h4>
          <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      <div class="w-full lg:w-80 flex-shrink-0 space-y-6">
        ...
        <div class="bg-white dark:bg-slate-900 rounded-xl shadow p-6">
          ...
          <div class="flex flex-wrap items-center gap-2 mt-2">
            <span v-for="(skill, idx) in skills" :key="idx" class="px-2 py-0.5 rounded-full bg-violet-100 text-violet-700 text-xs font-medium border border-violet-200">
              {{ skill }}
            </span>
          </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-xl shadow p-6">
          ...
          <div class="space-y-2 text-sm">
            <div class="flex items-center gap-2">
              <i class="mdi mdi-map-marker-radius text-violet-600"></i>
              <span>Radius layanan antar jemput: <b>10 km</b> dari Toko</span>
            </div>
            <div class="flex items-center gap-2">
              <i class="mdi mdi-clock-outline text-violet-600"></i>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-xl shadow p-6">
          <h4 class="font-bold text-lg mb-2 flex items-center gap-2">
            <i class="mdi mdi-message-question text-violet-600 text-xl"></i>
            Konsultasi Gratis
          </h4>
          ...
          <form @submit.prevent="onConsultSubmit" class="space-y-3">
            <textarea class="w-full rounded-lg border border-violet-200 dark:border-slate-700 p-2 text-sm" rows="3" placeholder="Ceritakan masalah HP kamu..." v-model="consultMessage"></textarea>
